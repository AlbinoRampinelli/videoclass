...
export function CourseCard({ course, jaComprou, buttonType, onSaberMais }: CourseProps) {
  const liberado = jaComprou;
  const isSaberMais = buttonType === "saber-mais" && !liberado;
  ...
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-6 flex flex-col hover:border-[#81FE88]/30 transition-all group relative overflow-hidden">
      ...
      <div className={`absolute -top-10 -right-10 w-32 h-32 rounded-full blur-3xl transition-all ${
        isSaberMais ? "bg-zinc-800/20" : "bg-[#81FE88]/5 group-hover:bg-[#81FE88]/10"
      }`} />

      <div className="flex justify-between items-start mb-6">
        <div className={`p-3 border rounded-xl transition-transform group-hover:scale-110 ${
          isSaberMais ? "bg-zinc-800/50 border-zinc-700 text-zinc-400" : "bg-zinc-800/50 border-zinc-700 text-[#81FE88]"
        }`}>
      ...
      {isSaberMais ? (
        ...
        <Link
          ...
          className={`w-full py-5 rounded-2xl font-black text-center transition-all uppercase shadow-xl ${
            liberado
