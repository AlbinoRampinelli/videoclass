            ...
            <div className="pt-4">
              <button
                onClick={(e) => {
                  ...
                  // Se o sinal oficial chegou, usa ele
                  if (onAction && typeof onAction === 'function') {
                    onAction();
                    return;
                  }

                  // Se o sinal não chegou (seu caso atual), vamos forçar a Vitrine a abrir o CPF
                  console.log("Forçando abertura do CPF via evento...");

                  // Criamos um sinal global que a Vitrine vai ouvir
                  const eventoCpf = new CustomEvent('abrirRegistroManual');
                  window.dispatchEvent(eventoCpf);

                  // Fecha o modal atual para não ficar um em cima do outro
                  if (onClose) onClose();
                }}
